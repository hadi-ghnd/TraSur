...
categories:
    0) person 1) car 2) bus 3) cyclist 4) truck
"""
...
for fileName in os.listdir(src_lbl_path):
    ...
    for line in lines:
        ...
        category = lst[0]
        if category == '2': continue # we have enough cars
        ...
        if category == '4': # if it's a truck just add one image
            ...
            cnt += 1
            cv.imwrite(os.path.join(src_img_path, imgName + extension), newImage)
            with open(os.path.join(src_lbl_path, imgName + '.txt'), 'w') as f1:
                f1.write(line)
            ...
            for i in range(len(transforms)):
                ...
                imgName = BaseFileName + "_" + "frame" + "_" + "{:06d}".format(cnt)
                cnt += 1
                cv.imwrite(os.path.join(src_img_path, imgName + extension), transformed_image)
                with open(os.path.join(src_lbl_path, imgName + '.txt'), 'w') as f1:
                    f1.write(line)
